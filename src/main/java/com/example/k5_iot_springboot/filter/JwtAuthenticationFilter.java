...
/*
    ...
    : JWT 인증 필터
    - 요청에서 JWT 토큰을 추출
    - request의 header에서 토큰을 추출하여 검증 (유효한 경우 SecurityContext에 인증 정보 설정)
...
//    === 상수 & 필드 선언 === // 
     ...
     * >> Spring Security Filter가 매 요청마다 호출하는 핵심 메서드
     * 
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
//            0) 사전 스킵 조건: 이미 인증된 컨텍스트가 있으면 그대로 진행(스킵) (다른 필터가 인증 처리를 한 경우, 중복 인증 방지)
...
//        UsernamePasswordAuthenticationToken 클래스는 Spring Security 에서 자주 쓰이는
//              , "인증 토큰 구현체"
...
//        빈 SecurityContext 객체 생성 - 인증 토큰 주입
        ...
        context.setAuthentication(authenticationToken); // 방금 만든 인증 토큰을 전달
...
//        +) hasAuthority("권한")는 명시적 문자열 그대로 권한을 확인
        }
