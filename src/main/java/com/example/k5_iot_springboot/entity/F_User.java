...
//    private Set<RoleType> roles = new HashSet<>();
    /*
    * ===== 권한 컬렉션 (조인 엔티티) =====
    * mappedBy = "user": F_UserRole 엔티티 안의 user 필드가 연관관계의 주인을 뜻함
    * cascade = CascadeType.ALL: F_UserRole 생성/삭제 전파
    * orphanRemoval = true: 컬렉션에서 제거되면 join row 삭제
    * */
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL, orphanRemoval = true)
    private Set<F_UserRole> userRoles = new HashSet<>();
...
//    1. 이미 같은 role이 있는 경우 중복 추가 X
    public void grantRole(F_Role role) {
        boolean exists = userRoles.stream().anyMatch(ur -> ur.getRole().equals(role));
        if(!exists) {
//            존재하지 않는 Role이 추가된 경우(정상 작동)
            userRoles.add(new F_UserRole(this, role));
        }
    }

//    2. 권한 회수
    public void revokeRole(F_Role role) {
