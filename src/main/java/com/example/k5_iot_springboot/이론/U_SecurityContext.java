...
/*
    ...
    4. 사용자 로딩 (UserDetailsService)
        ...
        - UserRepository.findByLoginId(loginId)로 F_User 엔티티 조회
        - 조회 실패) UsernameNotFoundException 발생 ("아이디/비밀번호가 올바르지 않음"으로 외부 전달)

    5. 보안 표현으로 매핑
        - 조회된 F_User -> UserPrincipalMapper.map(user)
    ...
    11. 응답 작성
        - 헤더 Authorization: Bearer <토큰> 또는 Body JSON 반환
...
*/
public class U_SecurityContext {
}
