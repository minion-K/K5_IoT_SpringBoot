...
/*
    ...
    - 내장 WAS(Tomcat 등)를 포함: 별도 설피 없이 실행 가능
    - JAR 파일만으로 서버 실행 및 배포 가능 (외부 서버 설치 불필요)
    
    3. 스프링 부트의 동작 구조
    
    [ 클라이언트 ] - [ 서블릿 컨테이터 ] - [ Dispatcher Servlet ]
                                                |
    ...
    1) 클라이언트 >> 서블릿 컨테이너 >> Dispatcher Servlet(DS) 
        : 클라이언트에서 "요청"이 들어오면, 서블릿 컨테이너(Tomcat)가 URL 매핑에 따라
            해당 요청을 DS로 전달
...
/*
    ...
    2. 스프링부트(MVC)
        1) DispatcherServlet 하나가 관문
            ...
            - 공동 보일러 플레이트(바인딩, 검증, 예외, 직렬화)를 프레임워크가 제공
        2) 서블릿 레벨이 DispatcherServlet 1개면 충분(스프링이 자동 등록)
            - 컨트롤러 클래스/메서드는 여러 개
                >> 애플리케이션 코드 레벨
                    "서블릿 여러 개가 아니라" 아니라 POJO(Plain Old Java Object) 단위가 가볍고 일관적
