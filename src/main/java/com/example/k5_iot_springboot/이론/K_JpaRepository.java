...
/*
    ...
    - 반환타임 메서드명(매개변수);의 추상 메서드 형식

    1. CRUD 메서드
    1) Entity save(Entity entity);
        : 새로운 엔티티를 저장하거나, 기존 엔티티를 업데이트
        - 매개변수의 데이터 값 중 ID 값이 DB에 존재하는 경우 update / 존재하지 않을 경우 insert
        - 저장 또는 수정 후 반영된 데이터를 반환
        
    2) Optional<Entity> findById(Id id);
        : 주어진 ID에 해당 하는 엔티티를 조회
    ...
    반환타입 fingBy필드명(필드타입 값);
    반환타입 fingBy필드명AND다른필드명(필드타입 값1, 필드타입 값2);
...
public interface K_JpaRepository {
