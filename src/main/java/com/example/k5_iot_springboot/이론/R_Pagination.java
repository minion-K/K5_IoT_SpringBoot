...
/*
    ...
    - UX: 목록을 나누어 보여주거나 "더보기/무한 스크롤" 제공
    ...
    2. 페이지네이션 기본 용어
        ...
        - Page VS Slice
            >> Page: 전체 개수(count)를 포함 ("총 몇 페이지인지" 같은 정보가 필요할 때)
            >> Slice: 전체 개수 없이 "다음 페이지가 더 있는지(hasNext)"만 판단
                    (성능 유리 - 무한 스크롤/더보기에 적합)

    3. 페이지네이션 종류(2가지)
        1) Offset 기반(page/size/정렬)
            - 요청: /boards?page=0&size=10&sort=createAt,desc
            ...
            +) 페이지 번호(page)와 크기(size)로 건너뛸 개수(offset = page X size)를 계산하여
                LIMIT size OFFSET offset(값)

        2) Cursor(Keyset) 기반(cursorId/size)
            - 요청: /boards/cursor?cursorId=마지막아이템id&size=10
            - 장점: 빠름(대용량에 유리), 인피니트 스크롤에 최적
            - 단점: "총 페이지 수" 계산 어려움, 정렬을 특정 키로 고정(id 또는 createAt + id)
            >> 데이터가 클 경우 무한 스크롤/피드형 목록, 실시간성의 성능이 중요한 화면

            +) 마지막으로 본 키 이후 데이터를 키 조건(id < lastId)으로 조회
                앞부분을 통째로 건너뛰지 않음
