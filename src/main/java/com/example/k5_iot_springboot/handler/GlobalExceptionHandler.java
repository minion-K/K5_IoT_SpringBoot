...
// 스프링이 빈으로 등록 - 해당 프로젝트 전역의 @RestController 에서 발생하는 예외 처리
...
//        1) reason 설정: 비워질 경우 ErrorCode의 기본 메시지 값 사용
...
//        2) 클라이언트가 해석할 수 있는 표준 에러 응답 본문 조립
        ...
        return ResponseEntity.status(code.status)
                .body(ResponseDto.setFail(finalReason, body));
    }
...
//    === 409 Conflict: 무결성 위반(중복/제약조건)
    @ExceptionHandler({DataIntegrityViolationException.class}) // Unique가 충돌 FK 위반 등
...
//    === 500 Internal Server Error: 그 밖의 모든 예외에 대한 최종 안전망 ===
