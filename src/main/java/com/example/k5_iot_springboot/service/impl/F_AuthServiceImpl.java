...
public class F_AuthServiceImpl implements F_AuthService {
...
//        1) 유효성 검사(중복 체크 - 유니크 제약 검증)
        ...
        if(userRepository.existsByEmail(req.loginId())) {
            throw new IllegalArgumentException("이미 사용 중인 이메일입니다.");
        }
        if(userRepository.existsByNickname(req.loginId())) {
...
//        3) 엔티티 생성 / 저장
        F_User user = F_User.builder()
                .loginId(req.loginId())
                .password(encoded)
                .email(req.email())
                .nickname(req.nickname())
//                .gender(req.gender()) - 선택사항(null 허용)
                .build();


//    기본 권한 부여
//    cf) getReferenceById: 특정 ID를 가진 엔티티의 프록시 객체를 즉시 반환
    F_Role defaultRole = roleRepository.getReferenceById(RoleType.USER);
