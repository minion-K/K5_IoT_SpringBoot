 ...
 * - "엔티티(F_User) 자체를 지니지 않고", 인증이 필요한 값만 안전하게 전달/보관
 *      >> 결합도를 낮춤, 캐시/직렬화 안정성 향상
 *      
 * 필요성
 * 1) Security의 표준 진입점: AuthenticationProvider는 UserDetails 타입을 통해 사용자 정보와 권한을 검사
 *      >> CustomUserDetailsService#loadByUsername()에서 DB 사용자 정보를 읽고 해당 클래스로 감싼 뒤 반환하면
 *          , 이후 인증 과정이 표준화되어 동작
 *          
 ...
 *  - Spring Security의 hasRole("USER") / hasAuthority("ROLE_USER") 검사와 호환되도록 "ROLE_" 접두어를 붙이는 것을 권장
 *
 * >> 설계 포인트
...
//    UserDetails: Security가 요구하는 사용자 정보 인터페이스
...
//    : 계정의 비밀번호 리턴
//    >> 인증 단계에서 DaoAuthenticationProvider가 비밀번호 매칭에 사용 (반드시 해시 값)
