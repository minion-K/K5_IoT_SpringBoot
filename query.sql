...
# 0910 (F_UserRoleId)
-- 사용자-권한 매핑(조인 엔티티)
-- DROP TABLE IF EXISTS `user_roles`; # 기존의 user_roles 엔티티 제거
CREATE TABLE IF NOT EXISTS `user_roles` (
	user_id 	BIGINT NOT NULL,
...
# 0901 (주문 관리 시스템)
-- 트랜잭션, 트리거, 인덱스, 뷰 학습
# products(상품), stocks(재고), orders(주문 정보)
#, order_items(주문 상세 정보), order_logs(주문 기록 정보)

-- 안전 실행: 삭제 순서
# FOREIGN_KEY_CHECKS: 외래키 제약 조건을 활성화(1)하거나 비활성화(0)하는 명령어
...
CREATE TABLE IF NOT EXISTS `products` (
  ...
  DEFAULT CHARSET = utf8mb4						# DB나 테이블의 기본 문자 집합(4byte까지 지원 - 이모지 포함)
  COLLATE = utf8mb4_unicode_ci					# 정렬 순서 지정(대소문자 구분 없이 문자열 비교 정렬)
  COMMENT = '상품정보';
  
# cf) ENGINE = InnoDB: 트랙잭션 지원(ACID), 외래 키 제약 조건 지원(참조 무결성 보장)
...
CREATE TABLE IF NOT EXISTS `order_logs` (
    ...
    INDEX idx_order_logs_create_at (created_at)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci
  COMMENT = '주문 기록 정보';
...
FROM
    ...
    JOIN order_items oi ON o.id = oi.order_id
    JOIN products p ON oi.product_id = p.id
GROUP BY o.id, o.user_id, o.order_status; -- 주문별 합계: 주문(orders) 정보를 기준으로 그룹화
